    ...
    ifNotTester(() async {
      if (addReaction) {
        if (commentReactionList.length < 3 && isReacted.validate()) {
          if (commentReactionList.any((element) => element.user!.id.validate().toString() == userStore.loginUserId)) {
            int index = commentReactionList.indexWhere((element) => element.user!.id.validate().toString() == userStore.loginUserId);
            commentReactionList[index].id = reactionID.validate().toString();
            commentReactionList[index].icon = appStore.reactions.firstWhere((element) => element.id == reactionID.validate().toString().validate()).imageUrl.validate();
            commentReactionList[index].reaction = appStore.reactions.firstWhere((element) => element.id == reactionID.validate().toString().validate()).name.validate();
          } else {
        ...
        commentReactionList.removeWhere((element) => element.user!.id.validate().toString() == userStore.loginUserId);
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Row(
              children: [
                if (appStore.isReactionEnable&& appStore.reactions.validate().isNotEmpty && widget.comment.id != null)
                  ReactionButton(
                    isComments: true,
                    isReacted: isReacted,
