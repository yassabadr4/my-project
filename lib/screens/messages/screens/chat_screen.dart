        ...
        appBar: AppBar(
          ...
          actions: [
            if (selectedMessage != null)
              Row(
                children: [
                  if (thread!.permissions!.canReply.validate() && selectedMessage!.senderId.toString() != userStore.loginUserId)
                    InkWell(
                      onTap: () {
                  ...
                  if (thread!.permissions!.canEditOwnMessages.validate() && selectedMessage!.senderId.toString() == userStore.loginUserId)
                    InkWell(
                      onTap: () {
                        replyMessage = selectedMessage;
                        isEditMessage = true;
                  ...
                  if (thread!.permissions!.canDeleteAllMessages.validate() || (thread!.permissions!.canDeleteOwnMessages.validate() && selectedMessage!.senderId.toString() == userStore.loginUserId))
                    InkWell(
                      onTap: () {
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      MessageMeta? meta;
                      bool isLoggedInUser = message.senderId.toString() == userStore.loginUserId;
                      ...
                      /// From Api created_at value has 14 digits which is MicrosecondsSinceEpoch and if we generate MicrosecondsSinceEpoch in dart it will be off 16 digit
                      String lastDate = index != 0 ? DateFormat(DATE_FORMAT_3).format(getDateTimeFromPhpTimeStamp(messages.validate()[index - 1].createdAt.validate())) : "";
                      String date = DateFormat(DATE_FORMAT_3).format(getDateTimeFromPhpTimeStamp(message.createdAt.validate()));
                        ...
                        if (meta.reactions != null && meta.reactions.validate().isNotEmpty) {
                          if (thread!.type == 'group' || thread!.participantsCount.validate() > 2) {
                            meta.reactions.validate().forEach((element) {
                              if (element.users.validate().contains(userStore.loginUserId.toInt())) {
                                int index = meta!.reactions.validate().indexWhere((e) => e.users.validate().contains(userStore.loginUserId.toInt()));

                                reactionList.validate().forEach(
                                  (element) {
                                    if (element.emojiId.validate() == meta!.reactions.validate()[index].reaction.validate()) {
                                      int iconIndex = appStore.emojiList.indexWhere((e) => e.skins.validate().first.unified.validate() == element.emojiId.validate());
                        ...
                        return Column(
                          children: [
                            ...
                            ChatScreenMessagesComponent(
                              ...
                              message: message,
                              meta: meta,
                              time: message.lastUpdate.validate() == 0 ? timeStampToDateMessage(message.createdAt.validate()) : timeStampToDateMessage(message.lastUpdate.validate()),
