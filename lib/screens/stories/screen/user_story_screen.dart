...
class _UserStoryScreenState extends State<UserStoryScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> getStories() async {
    if (showHighlightStory) {
      ...
      appStore.setLoading(true);
      list.clear();
      await getUserStories(userId: userStore.loginUserId.toInt()).then((value) {
        list.addAll(value);
        setState(() {});
        appStore.setLoading(false);
      }).catchError((e) {
          ...
          Theme(
            ...
            child: PopupMenuButton(
              ...
              itemBuilder: (context) => <PopupMenuEntry>[
                ...
                if (currentValue != 3 && appStore.storyActions.validate().any((element) => element.action == StoryHighlightOptions.trash))
                  PopupMenuItem(
                    value: 3,
                    child: Text(language.draftStories, style: primaryTextStyle()),
                  ),
                if (currentValue != 4 && appStore.storyActions.validate().any((element) => element.action == StoryHighlightOptions.trash))
                  PopupMenuItem(
                    value: 4,
